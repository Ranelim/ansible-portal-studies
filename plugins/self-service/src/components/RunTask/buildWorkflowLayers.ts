 ...
 */
export type AapLogEntry = {
  id: number;
  label: string;
  status?: string;
  hasPlaybookOutput: boolean;
  content?: string;
  loading?: boolean;
};
...
export function extractWorkflowEdges(
  ...
  const ids = new Set(nodes.map(n => Number(n.id)));
  for (const node of nodes) {
    const fromId = Number(node.id);
    for (const t of normalizeWorkflowEdgeTargets(node.success_nodes)) {
      if (ids.has(t)) edges.push({ from: fromId, to: t, type: 'success' });
    }
    for (const t of normalizeWorkflowEdgeTargets(node.failure_nodes)) {
      if (ids.has(t)) edges.push({ from: fromId, to: t, type: 'failure' });
    }
    for (const t of normalizeWorkflowEdgeTargets(node.always_nodes)) {
      if (ids.has(t)) edges.push({ from: fromId, to: t, type: 'always' });
    }
  }
  return edges;
}
