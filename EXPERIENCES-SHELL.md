...
Quality remediation session = Inline visual 3-step (`FORCED_REMEDIATION_WIZARD = 'visual'`), Continue under the stepper (`FORCED_CTA_LAYOUT = 'current'`). Auto-fix / AI-fix / Not fixable tabs; AI quota on the AI-fix tab. Prototype wizard and sticky-footer compare parked. **Current / Current redesign / Work-first** layout toggle on Results & Remediation only (`SHOW_REVIEW_LAYOUT_COMPARE`, `?review=redesign` or `?review=work`).
