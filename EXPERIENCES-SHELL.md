...
| Done (park) | Quality remediation **Inline visual** forced (`FORCED_REMEDIATION_WIZARD = 'visual'`), Continue under the stepper (`FORCED_CTA_LAYOUT = 'current'`). Findings use Auto-fix / AI-fix / Not fixable tabs; AI spend is on the AI-fix tab (Lightspeed quota, no token/$ estimates). Prototype wizard and sticky-footer / Wizard footer compares stay hidden. **Results & Remediation** shows a **Current / Current redesign / Redesign 3 / Redesign 4** layout toggle (`?review=redesign`, `?review=redesign3`, or `?review=redesign4`). Current redesign is a fork of Current. Redesign 3 copies that fork. Redesign 4 copies Redesign 3 with a compact **Accept / Decline** toggle (undecided / accepted / declined), **Auto-accept all auto-fixes**, and **Accept remaining** (undecided only). Set `SHOW_REVIEW_LAYOUT_COMPARE = false` to hide the strip. Set either force `null` to revive `?wizard=` / `?cta=`. |
...
Quality remediation session = Inline visual 3-step (`FORCED_REMEDIATION_WIZARD = 'visual'`), Continue under the stepper (`FORCED_CTA_LAYOUT = 'current'`). Auto-fix / AI-fix / Not fixable tabs; AI quota on the AI-fix tab. Prototype wizard and sticky-footer compare parked. **Current / Current redesign / Redesign 3 / Redesign 4** layout toggle on Results & Remediation only (`SHOW_REVIEW_LAYOUT_COMPARE`, `?review=redesign`, `?review=redesign3`, or `?review=redesign4`).
