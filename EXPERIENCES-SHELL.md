...
Quality remediation session = Inline visual (`FORCED_REMEDIATION_WIZARD = 'visual'`): Scan → Results → Auto remediations → AI remediations (skip if none) → Commit. Scan auto-advances to Results. Results is all findings, read-only. Continue under the stepper (`FORCED_CTA_LAYOUT = 'current'`). No Manual step. Prototype wizard, sticky-footer, and layout compare parked. **Redesign 4** forced (`FORCED_REVIEW_LAYOUT`). **Steps compare** on `/apme/remediate`: **With Results** (default) · **Results and auto remediations** (`?steps=work`) · **Review** (`?steps=review`) — stepper **Scan → Results and auto remediations → AI remediations → Commit**; inventory of all findings (12 + 8 auto remediations · 2 need AI · 2 manual only) sits in the **list box below the stepper** (title is **org/repo**, same as Combined; subtitle is **Health scan: timestamp · commit SHA**; Back keeps the destination), work list is autos (Accept all selected by default / Decline all toggle; row Accept / Decline stay two-state; no remaining; Include all omitted; footer **N included in the commit**; two-line clickable scan readout — findings · auto / need AI / manual · severity · category — filters the list; search, Content type, and work-list heading hidden), remainder cards below (Needs AI next step / Manual only). No severity chips. Rollback bundled one-page review (Scan → Results & Remediation with Generate AI on that page → Commit): tag `remediation-bundled-results-and-ai` (`b82fca95`). Rollback loved chrome (pre-nodes): `git reset --hard 239d2106`. Set `SHOW_REVIEW_LAYOUT_COMPARE = true` and `FORCED_REVIEW_LAYOUT = null` to revive Current / Current redesign / Redesign 3 / Redesign 4.
