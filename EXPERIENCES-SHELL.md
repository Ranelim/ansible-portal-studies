...
| Done (park) | Quality remediation **Inline visual** forced (`FORCED_REMEDIATION_WIZARD = 'visual'`), Continue under the stepper (`FORCED_CTA_LAYOUT = 'current'`). Findings use Auto-fix / AI-fix / Not fixable tabs; AI spend is on the AI-fix tab (Lightspeed quota, no token/$ estimates). Prototype wizard, sticky-footer, and **layout compare** parked (`SHOW_REVIEW_LAYOUT_COMPARE = false`). **Redesign 4** forced (`FORCED_REVIEW_LAYOUT = 'redesign4'`): three-state Accept / Decline, Auto-accept all auto-fixes, Accept remaining (undecided only). Coupled findings on the same ContentGraph path share one Accept / Decline (cards stay “findings”; no “node” label). Current / Current redesign / Redesign 3 stay in code. UI before node grouping: `239d2106`. Set `SHOW_REVIEW_LAYOUT_COMPARE = true` and `FORCED_REVIEW_LAYOUT = null` to revive `?review=`. Set wizard/CTA force `null` to revive `?wizard=` / `?cta=`. |
